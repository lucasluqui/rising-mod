...
class report:
    ...
    async def report(self, ctx, heroName, evidence, *, reason):
        ...
        if "player" in [y.name.lower() for y in ctx.message.author.roles] and not "muted" in [y.name.lower() for y in ctx.message.author.roles]:
            ...
            # Tells the user the vote ban was sent successfully.
            await self.bot.say('Your report was sent successfully, <@{}>.'.format(str(ctx.message.author.id)))
            
            # Sends the vote ban to the channel.
            callout_channel = self.bot.get_channel(settings['votebans_channel'])
            report_embed = discord.Embed(title="VOTE BAN!", description='Hello community! Please watch the evidence below and vote on the suspect, thanks.', colour=discord.Colour.red())
            ...
            report_embed.set_footer(text='Vote ban sent by: {}.'.format(str(ctx.message.author)))
            ...
            vbanlog_embed = discord.Embed(title="VOTE BAN LOG", description='Please decide the fate of this user.', colour=0x5f8fcc)
            ...
            vbanlog_embed.set_footer(text='This vote ban was originally sent by: {}. Date: {}.'.format(str(ctx.message.author), str(vban_date)))
            ...
            report_embed_ended = discord.Embed(title="VOTE BAN ENDED!", description='Thanks everyone for submitting their votes.', colour=discord.Colour.orange())
            ...
            report_embed_ended.set_footer(text='Originally sent by: {}.'.format(str(ctx.message.author)))
        ...
        # If they're not linked, they won't be able to send vote bans.
           ...
           await self.bot.say("Sorry <@{}>, you need to link your discord account in order to report.".format(str(ctx.message.author.id)))
