...
class tempMute:
    ...
    async def tempmute(self, ctx, member:discord.Member, mutedTime, *, reason):
        ...
        if "team" in [y.name.lower() for y in ctx.message.author.roles] and totalMutedTime <= 86400 and not "muted" in [y.name.lower() for y in member.roles] and member.id != settings['botid']:
            ...
            # Announces mute.
            await self.bot.say("**<@{}>** was successfully muted for **{}** minutes.".format(member.id, mutedTime))
            ...
            try:
                await self.bot.send_message(member, "Your temporary mute has ended. Please watch your behavior and follow the rules.")
            except:
                pass
