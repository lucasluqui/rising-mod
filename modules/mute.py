...
class mute:
    ...
    async def mute(self, ctx, member:discord.Member, *, reason):
        ...
        if "team" in [y.name.lower() for y in ctx.message.author.roles] and not "muted" in [y.name.lower() for y in member.roles] and member.id != settings['botid']:
            ...
            # Announces mute.
            await self.bot.say("**<@{}>** was successfully muted.".format(member.id))
        ...
        # Prevent clever ones from trying to use the bot... to mute itself.
    ...
    async def unmute(self, ctx, member:discord.Member):
        ...
        if "team" in [y.name.lower() for y in ctx.message.author.roles] and "muted" in [y.name.lower() for y in member.roles]:
            ...
            await self.bot.say("**<@{}>** was successfully unmuted.".format(member.id))

            # Tries to message to the unmuted user via DM.
            try:
                await self.bot.send_message(member, "You have been unmuted by **{}#{}**. Please watch your behavior and follow the rules.".format(str(ctx.message.author.name), str(ctx.message.author.discriminator)))
            except:
                pass
